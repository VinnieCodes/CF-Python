...
def create_recipe():
  try:
    ...
    # Check that anme is alphanumeric and less than 50 characters
    while not is_alpha_space_or_hyphen(name) or len(name) > 50:
      print()
      print(
        "Please enter a recipe name that contains only letters, spaces, and hyphens and is less than 50 characters"
      )
      name = input("\nEnter the name of the recipe: ")

    # Ask user for cooking time
    cooking_time = input("\nEnter the cooking time in minutes: ")
    # Check if cook time is numeric
    ...
    for i in range(number_of_ingredients):
      ...
      # Check that ingredient is alpabetical
    ...
    # Join ingredients list in a string separated by commas
    ...
    # Caculate the difficulty of the recipe
    ...
    print()
    print("There was a n error creating the recipe")
    print(e)
    print()
...
def view_all_recipes():
  ...
  # If there are no recipes, infrom user and exit function
  if len(recipes) == 0:
    print()
...
def search_by_ingredient():
  # Retrieve all ingredients from database and stor in a list
  ...
  print()
  # Ask user to enter the number of the ingredients they want to search for
  search_ingredients = input("Enter the numbers of the ingredients you want to search for separated by spaces: ").split()
  # Validate user input
  for i in search_ingredients:
    if not i.isnumeric() or int(i) >len(all_ingredients):
      ...
      print()
      return None
  # Get recipes that contain the ingredients
...
def edit_recipe():
  ...
  while not recipe_id.isnumeric() or int(recipe_id) > len(results):
    ...
    print("Please enter a valid number")
    recipe_id = input("Enter the number of the recipe you want to edit: ")
  # Get the recipe to edit and display the recipe, ingredients and cooking time
  ...
  if attribute == "1":
    ...
    while not is_alpha_space_or_hyphen(new_name) or len(new_name) > 50:
      print()
      print("Please enter a recipe name that is alphanumeric and less than 50 characters")
      new_name = input("Enter the new name: ")
    ...
    for i in range(int(num_ingredients)):
      ...
      while not is_alpha_space_or_hyphen(ingredient) and ingredient != "done":
        print()
        print("Please enter an ingredient using only letters, spaces or hyphens")
        ingredient = input(f"Enter ingredient {i+1} or type 'done' to finish: ")
        if ingredient != "done":
...
def delete_recipe():
  ...
  results = session.query(Recipe.id, Recipe.name).all()
  print()
  print("Here are the recipe(s) in the database:")
  print()
  for result in results:
    print(f"{result[0]}. {result[1]}")
  print()
